...
\noindent \textbf{Resultados:} O modelo bivariado estimou sensibilidade agrupada de 78,1\% (IC 95\%: 54,9--91,3\%) e especificidade agrupada de 96,8\% (IC 95\%: 89,1--99,1\%), com área sob a curva (AUC) SROC de 0,953, razão de verossimilhança positiva (RV+) de 24,10 e razão de verossimilhança negativa (RV-) de 0,226. O intervalo de predição de 95\% para a sensibilidade foi amplo, variando de 6,4\% a 99,5\% ($\tau_{\text{sens}}=0,885$), indicando alta heterogeneidade entre os estudos. Na análise de sensibilidade que excluiu o estudo de maior peso amostral (Huang 2025, N=97.651, prevalência = 0,03\%), a sensibilidade foi de 79,2\% (IC 95\%: 52,8--92,8\%) e a especificidade de 93,7\% (IC 95\%: 91,0--95,6\%), com RV+ de 12,54 e AUC de 0,950. Apenas 9 dos 12 estudos identificados disponibilizaram dados brutos para pooling formal.
...
\noindent \textbf{Methods:} The review was conducted in accordance with PRISMA-DTA guidelines. Methodological quality was assessed using QUADAS-2. The hierarchical bivariate random-effects model (Reitsma et al.) was estimated by REML for studies with complete raw 2$\times$2 data. Nine primary studies were included in the bivariate analysis, totaling 113,714 radiographs.
...
Este estudo constitui uma revisão sistemática com meta-análise de acurácia diagnóstica conduzida em estrito alinhamento com as recomendações do \textit{Preferred Reporting Items for Systematic Reviews and Meta-Analyses for Diagnostic Test Accuracy} (PRISMA-DTA) \cite{mcinnes} e os princípios do \textit{Standards for Reporting Diagnostic Accuracy} (STARD). Para subsidiar a análise de modelos multimodais de imagem aplicados ao tórax, recorreu-se aos princípios norteadores recém-propostos do framework PRIME 2.0 (\textit{Proposed Requirements for Cardiovascular Imaging-Related Multimodal AI Evaluation}), que estabelece diretrizes para a transparência do reporte de prompts, parâmetros de inferência e controle de alucinações em IAs generativas de imagem médica. O protocolo não foi registrado prospectivamente no PROSPERO devido ao caráter exploratório e à constatação de elevada heterogeneidade de dados no levantamento inicial da literatura primária. O protocolo desta revisão sistemática foi posteriormente registrado no Open Science Framework (OSF) sob o DOI: 10.17605/OSF.IO/U6BJS. Todos os dados, códigos analíticos e materiais suplementares utilizados para garantir a reprodutibilidade desta pesquisa estão publicamente disponíveis no repositório GitHub, espelhados no projeto OSF (\url{https://osf.io/sh5z7}).

\subsection{Estratégia de Busca e Critérios de Elegibilidade (PIRT)}
A busca sistemática de artigos foi realizada nas bases PubMed/MEDLINE, SciELO e BVS abrangendo o período de 2018 a 2026. A triagem inicial dos registros recuperados foi automatizada via pipeline de Processamento de Linguagem Natural (PLN) baseada em Term Frequency-Inverse Document Frequency (TF-IDF), seguida de avaliação cega por revisores independentes. A elegibilidade dos estudos primários baseou-se nos critérios PIRT:
\begin{itemize}
...
Visando contornar a indeterminação matemática de células nulas nas matrizes 2$\times$2 individuais (divisão por zero na escala logit), aplicou-se a correção de continuidade de Haldane-Anscombe, adicionando-se o valor constante de 0,5 a todas as células da matriz do estudo de Ciflik et al. (2026) \cite{ciflik}, que relatou zero falsos negativos (sensibilidade bruta de 100,0\%). O procedimento resultou em uma sensibilidade corrigida de 98,4\% (30,5/31), garantindo a estabilidade e a convergência numérica do algoritmo de otimização BFGS do modelo de Reitsma.
...
\begin{itemize}
    ...
    \item \textbf{Cenário D:} Exclusão de estudos classificados com alto risco de viés em qualquer domínio do QUADAS-2 (Akçay 2025 \cite{akcay} e Khovanova 2025 \cite{khovanova});
...
Khovanova 2025 \cite{khovanova} & VLM & Nódulo Pulmonar & 83 & 12 & 3 & 26 & 42 \\
\bottomrule
\end{tabular}
}
\begin{singlespace}
...
Khovanova 2025 & 31,6\% & 93,3\% & 12 & 3 & 26 & 42 & 65,1\% \\
\bottomrule
\end{tabular}
\begin{singlespace}
\centering\small Fonte: Elaborado pelo autor (2026), com base nos estudos primários citados.
...
Hong 2025c \cite{hong2025c} & Baixo & Baixo & Baixo & Baixo & Baixo & Baixo & Baixo \\
...
Khovanova 2025 & Alto & Incerto & Baixo & Incerto & Baixo & Baixo & Baixo \\
...
\begin{figure}[H]
    \centering
    \caption{QUADAS-2: Resumo por Domínio (N=13).}
...
O intervalo de predição de 95\% estimado para a sensibilidade foi de 6,4\% a 99,5\%, enquanto para a especificidade variou de 18,1\% a 100,0\% ($\tau_{\text{sens}}=0,885$; $\tau_{\text{fpr}}=2,218$). A elevada amplitude dos intervalos de predição evidencia a alta variabilidade e heterogeneidade intrínseca do desempenho de leitura visual entre os diferentes modelos avaliados no pool, indicando que não é possível assegurar um patamar fixo de acurácia diagnóstica em novos ambientes sem controle rigoroso dos modelos.
...
A heterogeneidade quantificada pelo índice $I^2$ bivariado de Zhou-Dendukuri foi de 74,6\% no pool principal completo. Pelo método de Holling não ajustado, a variabilidade situou-se entre 57,8\% e 88,7\%, enquanto o método ajustado pelo tamanho amostral acusou heterogeneidade de apenas 1,7\% a 3,6\%. A acentuada divergência entre os métodos de Holling reflete o impacto da assimetria amostral causada por Huang 2025. O índice de Zhou-Dendukuri de 52,0\% obtido no Cenário C (sem ambos os outliers) confirma que a maior parte da heterogeneidade estatística é explicada pela dispersão inerente dos tamanhos amostrais e taxas de prevalência dos estudos primários. A análise do viés de publicação e da assimetria de relato foi realizada graficamente através do Funnel Plot apresentado na Figura \ref{fig:funnel}. A análise do viés de publicação e da assimetria de relato foi realizada graficamente através do Funnel Plot apresentado na Figura \ref{fig:funnel}.
...
VLM & 7 & 70,2\% & 92,8\% & Hong 2025a, Hong 2025b, Ostrovsky 2025, Akçay 2025, \\
    &   &        &        & Ciflik 2026, Güzel 2026, Khovanova 2025 \\
\bottomrule
\end{tabular}
}
\begin{singlespace}
...
O desempenho variou significativamente conforme a patologia avaliada, cujos estudos e exames por subgrupo de condição clínica estão descritos na Tabela \ref{tab:subgrupo_condicao}. Para a detecção de tuberculose (Hong 2025b), a sensibilidade individual foi elevada (95,2\%, com especificidade de 86,7\%), refletindo o bom desempenho do modelo em identificar padrões consolidation e infiltrados típicos de triagem epidemiológica. Para pneumonia (Ostrovsky 2025), a sensibilidade foi de 76,2\% e especificidade de 93,7\%. O pior desempenho geral ocorreu na detecção de nódulos pulmonares (Khovanova 2025), no qual a sensibilidade caiu para apenas 31,6\%, evidenciando a extrema dificuldade das IAs generativas em identificar lesões nodulares pequenas e isoladas.
...
Outras Condições & 4 & 2.783 & Hong 2025b, Ostrovsky 2025, Huang 2023, Khovanova 2025 \\
\bottomrule
\end{tabular}
}
\begin{singlespace}
...
\begin{enumerate}
    ...
    \item Viés de independência estatística pelo uso de dados originados do mesmo grupo e sistema (cluster Hong 2025a/b/c);
...
\bibitem{akcay} AKÇAY, O.; ÖZTÜRK, Ö. Evaluation of the effectiveness of the ChatGPT artificial intelligence application in the diagnosis of spontaneous pneumothorax on chest radiograph interpretation. \textit{BMC Pulmonary Medicine}, v. 26, n. 1, p. 8, 2026.
...
\bibitem{huang2025} HUANG, J.; MOOR, M.; BANERJEE, O. et al. Efficiency and Quality of Generative AI-Assisted Radiograph Reporting. \textit{JAMA Network Open}, v. 8, n. 4, p. e256797, 2025.

\bibitem{gunes} GUNES, Y. C.; CESUR, T. The Diagnostic Performance of Large Language Models and General Radiologists in Thoracic Radiology Cases: A Comparative Study. \textit{Journal of Thoracic Imaging}, v. 40, p. 12--19, 2025.
...
\bibitem{reitsma} REITSMA, J. B.; GLAS, A. S.; RUTJES, A. W. S. et al. Bivariate analysis of sensitivity and specificity produces informative summary measures in diagnostic reviews. \textit{Journal of Clinical Epidemiology}, v. 58, n. 10, p. 1082--1088, 2005.
...
\bibitem{tanno} TANNO, R.; BARRETT, D. G. T.; SELLERGREN, A. et al. Consensus, dissensus and uncertainty in medical AI. \textit{Nature Medicine}, v. 31, p. 110--117, 2025.
...
\bibitem{bai} BAI, Y.; ZHANG, R.; LEI, Y. et al. A DeepSeek-powered AI system for automated chest radiograph interpretation in clinical practice. \textit{Nature Communications}, v. 17, p. 72680, 2026.

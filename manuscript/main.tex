...
\begin{singlespace}
\small \noindent \textbf{Fonte:} Elaborado pelo autor (2026). \\
\textbf{Nota:} Sens = Sensibilidade agrupada; Spec = Especificidade agrupada; IC 95\% = Intervalo de Confiança de 95\%; RV+ = Razão de Verossimilhança Positiva; RV$-$ = Razão de Verossimilhança Negativa; AUC = Área sob a Curva SROC. O Cenário B (sem Huang 2025) corrige a distorção induzida pelo estudo de prevalência extremamente baixa sem incorrer no viés de exclusão da evidência desfavorável de Güzel 2026.
\end{singlespace}
\end{table}
